...
async function getPatientPromAutofillAnswers(consultSessionId: string): Promise<Record<string, DoctorAnswerValue> | null> {
  if (!prisma) {
    return null;
  }
...
export async function getSavedDoctorQuestionnaire(input: { consultSessionId?: string }) {
  ...
  try {
    ...
    if (!submission) {
      const autoAnswers = await getPatientPromAutofillAnswers(input.consultSessionId);
      if (!autoAnswers) {
        return null;
      }
...
export async function saveDoctorQuestionnaireToDatabase(record: DoctorQuestionnairePayload) {
  ...
  const autoPromAnswers = await getPatientPromAutofillAnswers(record.consultSessionId);
  const normalizedAnswers: Record<string, DoctorAnswerValue> = { ...record.answers };
  if (record.answers.odiPercent === undefined && record.answers.ndiPercent === undefined && autoPromAnswers) {
    Object.assign(normalizedAnswers, autoPromAnswers);
  }
