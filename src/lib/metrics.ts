...
async function getDoctorReviewLatencyMinutes() {
  ...
  try {
    ...
    console.warn("[metrics] Failed to calculate doctor review latency", error);
    return null;
  }
}
...
export async function getAdminSummary(): Promise<AdminSummary> {
  ...
  try {
    ...
    console.warn("[metrics] Failed to load admin summary", error);
    return {
      totalPatients: 0,
      completedPatients: 0,
      completionRate: 0,
...
export async function getUsageMetrics(): Promise<UsageMetric[]> {
  const fallbackMetrics: UsageMetric[] = [
    {
    ...
    return [
      {
        ...
        progress:
          averageDurationSeconds > 0 ? Math.min(100, Math.max(0, 100 - Math.round(averageDurationSeconds / 6))) : 0,
      },
      {
        label: "Weekly submissions",
        value: String(weeklySubmissions),
        note: "Submitted in last 7 days",
        progress: Math.min(100, weeklySubmissions * 8),
      },
      {
        label: "Autosave resume rate",
        value: `${autosaveResumeRate}%`,
        note: "Users continue from the last saved step",
        progress: autosaveResumeRate,
      },
      {
        label: "Doctor review latency",
        value: doctorReviewLatencyMinutes && doctorReviewLatencyMinutes > 0 ? `${doctorReviewLatencyMinutes}m` : "—",
        note: "Patient submit to doctor submit (average)",
        progress:
          doctorReviewLatencyMinutes && doctorReviewLatencyMinutes > 0
            ? Math.min(100, Math.max(0, 100 - Math.round(doctorReviewLatencyMinutes * 1.5)))
            : 0,
      },
    ];
  } catch (error) {
    console.warn("[metrics] Failed to load usage metrics", error);
    return fallbackMetrics;
  }
}